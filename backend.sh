...
CHECK_ROOT(){
 ...
 VALIDATE(){
  ...
  CHECK_ROOT
        ...
        cp /home/ec2-user/expense-shell.2/backend.service /etc/systemd/system/backend.service 
        # load the data before running the backend

        dnf install mysql -y &>>$LOG_FILE
        VALIDATE $? "Installing MySQL Client"     

        mysql -h mysql.nani30.online -uroot -pExpenseApp@1 < /app/schema/backend.sql &>>$LOG_FILE
